...
class canvas {
  public:
    ...
    canvas(uint32_t width, uint32_t height)
        : width_(width), height_(height), current_(static_cast<size_t>(width) * height),
    ...
    void clear_next() { std::fill(next_.begin(), next_.end(), cell{}); }

    /** @brief Copy current buffer to next buffer (for dirty-rect optimization). */
    void copy_current_to_next() { next_ = current_; }
     ...
     */
    template <typename Fn> void diff(Fn &&fn) const {
  ...
  private:
    [[nodiscard]] size_t index(uint32_t x, uint32_t y) const noexcept { return static_cast<size_t>(y) * width_ + x; }
