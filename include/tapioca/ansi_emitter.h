...
inline std::string cursor_up(uint32_t n = 1) {
    if (n == 0) return {};
    std::string s; s.reserve(12); s += "\033["; detail::append_uint(s, n); s += 'A'; return s;
}

/** @brief Move cursor down by n rows. */
inline std::string cursor_down(uint32_t n = 1) {
    if (n == 0) return {};
    std::string s; s.reserve(12); s += "\033["; detail::append_uint(s, n); s += 'B'; return s;
}

/** @brief Move cursor right by n columns. */
inline std::string cursor_right(uint32_t n = 1) {
    if (n == 0) return {};
    std::string s; s.reserve(12); s += "\033["; detail::append_uint(s, n); s += 'C'; return s;
}

/** @brief Move cursor left by n columns. */
inline std::string cursor_left(uint32_t n = 1) {
    if (n == 0) return {};
    std::string s; s.reserve(12); s += "\033["; detail::append_uint(s, n); s += 'D'; return s;
}
...
inline void append_fg_params(std::string& out, const color& c) {
    switch (c.kind) {
        case color_kind::default_color: out += "39"; break;
        case color_kind::indexed_16:
            if (c.r < 8) detail::append_uint(out, 30 + c.r);
            else detail::append_uint(out, 90 + (c.r - 8));
            break;
        case color_kind::indexed_256:
            out += "38;5;"; detail::append_uint(out, c.r); break;
        case color_kind::rgb:
            out += "38;2;"; detail::append_uint(out, c.r); out += ';';
            detail::append_uint(out, c.g); out += ';'; detail::append_uint(out, c.b); break;
    }
}

inline void append_bg_params(std::string& out, const color& c) {
    switch (c.kind) {
        case color_kind::default_color: out += "49"; break;
        case color_kind::indexed_16:
            if (c.r < 8) detail::append_uint(out, 40 + c.r);
            else detail::append_uint(out, 100 + (c.r - 8));
            break;
        case color_kind::indexed_256:
            out += "48;5;"; detail::append_uint(out, c.r); break;
        case color_kind::rgb:
            out += "48;2;"; detail::append_uint(out, c.r); out += ';';
            detail::append_uint(out, c.g); out += ';'; detail::append_uint(out, c.b); break;
    }
}
...
inline std::string set_title(std::string_view title) {
    std::string s; s.reserve(title.size() + 8);
    s += "\033]0;"; s += title; s += "\033\\"; return s;
}

inline std::string set_icon_name(std::string_view name) {
    std::string s; s.reserve(name.size() + 8);
    s += "\033]1;"; s += name; s += "\033\\"; return s;
}

inline std::string set_window_title(std::string_view title) {
    std::string s; s.reserve(title.size() + 8);
    s += "\033]2;"; s += title; s += "\033\\"; return s;
}
...
inline std::string report_cwd(std::string_view hostname, std::string_view path) {
    std::string s; s.reserve(16 + hostname.size() + path.size());
    s += "\033]7;file://"; s += hostname; s += path; s += "\033\\"; return s;
}

inline std::string hyperlink_open(std::string_view url, std::string_view id = {}) {
    std::string s; s.reserve(url.size() + id.size() + 16);
    s += "\033]8;";
    if (!id.empty()) { s += "id="; s += id; }
    s += ';'; s += url; s += "\033\\"; return s;
}

inline constexpr const char* hyperlink_close = "\033]8;;\033\\";

inline std::string notify(std::string_view message) {
    std::string s; s.reserve(message.size() + 8);
    s += "\033]9;"; s += message; s += "\033\\"; return s;
}

inline std::string set_foreground_color(uint8_t r, uint8_t g, uint8_t b) {
    char buf[40]; int n = std::snprintf(buf, sizeof(buf), "\033]10;rgb:%02x/%02x/%02x\033\\", r, g, b);
    return std::string(buf, static_cast<size_t>(n));
}
...
inline std::string set_background_color(uint8_t r, uint8_t g, uint8_t b) {
    char buf[40]; int n = std::snprintf(buf, sizeof(buf), "\033]11;rgb:%02x/%02x/%02x\033\\", r, g, b);
    return std::string(buf, static_cast<size_t>(n));
}
...
inline std::string set_cursor_color(uint8_t r, uint8_t g, uint8_t b) {
    char buf[40]; int n = std::snprintf(buf, sizeof(buf), "\033]12;rgb:%02x/%02x/%02x\033\\", r, g, b);
    return std::string(buf, static_cast<size_t>(n));
}
...
inline std::string clipboard_write(std::string_view clipboard, std::string_view base64_data) {
    std::string s; s.reserve(clipboard.size() + base64_data.size() + 12);
    s += "\033]52;"; s += clipboard; s += ';'; s += base64_data; s += "\033\\"; return s;
}

inline std::string clipboard_query(std::string_view clipboard) {
    std::string s; s.reserve(clipboard.size() + 12);
    s += "\033]52;"; s += clipboard; s += ";?\033\\"; return s;
}
...
inline std::string command_finished(int exit_code) {
    char buf[24]; int n = std::snprintf(buf, sizeof(buf), "\033]133;D;%d\033\\", exit_code);
    return std::string(buf, static_cast<size_t>(n));
}
...
public:
    ...
    void transition(const style& target, std::string& out) {
        ...
        if (delta.size() <= reset_path.size()) out += delta;
        else out += reset_path;
