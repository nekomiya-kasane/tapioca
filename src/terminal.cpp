...
terminal_caps terminal_caps::detect() noexcept {
    ...
    if (term && std::strcmp(term, "linux") == 0) {
        caps.supported_attrs = attr::bold | attr::dim | attr::underline | attr::blink | attr::reverse | attr::hidden;
        caps.unicode = false;
        return caps;
    }
    ...
    auto has_utf8 = [](const char *s) -> bool {
        ...
        return std::strstr(s, "UTF-8") || std::strstr(s, "utf-8") || std::strstr(s, "UTF8") || std::strstr(s, "utf8");
    };
    ...
    if (term && std::strstr(term, "xterm") && !std::strstr(term, "kitty")) {
        caps.supported_attrs = attr::bold | attr::dim | attr::italic | attr::underline | attr::blink | attr::reverse |
