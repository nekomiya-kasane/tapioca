...
namespace tapioca {
    ...
    static std::optional<input_event> win32_key(const KEY_EVENT_RECORD &ke) {
        ...
        if (ch != 0) {
            ...
            if (cp < 0x20 && sk == special_key::none) {
                return std::nullopt;
            }
    ...
    static std::optional<input_event> parse_kitty_csi_u(const char *buf, int n) {
        ...
        if (n < 4 || buf[0] != 27 || buf[1] != '[' || buf[n - 1] != 'u') {
            return std::nullopt;
        }
        ...
        if (mod_bits > 0) {
            ...
            if (m & 8) {
                mods = mods | key_mod::super_;
            }
            if (m & 16) {
                mods = mods | key_mod::hyper;
            }
            if (m & 32) {
                mods = mods | key_mod::caps_lock;
            }
            if (m & 64) {
                mods = mods | key_mod::num_lock;
            }
        }
    ...
    std::optional<input_event> poll_event(int timeout_ms) {
        ...
        if (n >= 2 && buf[0] == 27) {
            ...
            // Bare ESC
            if (n == 1) {
                return input_event{key_event{0, special_key::escape, key_mod::none}};
            }
        ...
        if (n == 1) {
            ...
            if (b == '\n' || b == '\r') {
                return input_event{key_event{0, special_key::enter, key_mod::none}};
            }
            if (b == '\t') {
                return input_event{key_event{0, special_key::tab, key_mod::none}};
            }
            if (b == 127) {
                return input_event{key_event{0, special_key::backspace, key_mod::none}};
            }
            // Ctrl+letter (1-26)
            if (b >= 1 && b <= 26) {
